...
public class ConditionParser {
    ...
    public ConditionParser(SlateLibrary slate) {
        this.slate = slate;
    }
    ...
    public List<Condition> parseConditions(ConfigurationNode config, String menuName) {
        ...
        for (ConfigurationNode condNode : config.childrenList()) {
            try {
                ...
                slate.getLogger().warning("Error parsing condition in menu " + menuName + " at path " + YamlLoader.toDotString(config.path()) + ".[" + index + "], see below for error:");
                e.printStackTrace();
            }
            index++;
        }
